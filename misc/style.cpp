...
//
// - Source files should never contain characters ouside the portable character
//   set as defined by POSIX (for compatibility).
//
// - Source files should never include TAB characters (for consistency of
//   presentation).
//
// - All macro names must consist entirely of upplercase letters, digits and
//   underscores, and they must contain at least two upper case letters.
//
// - A name that is valid for a macro (see above), is not valid for anything
//   other than a macro.


// Public macro names (those defined in public headers) must have `ARCHON_` as a
// prefix.
...
my_type my_func()
{
    // Put the opening brace of a function body on the next line below the
    // function prototype. This also applies to class member functions.

    // Put all other opening braces on the same line as the syntax element to
    // which the brace is subordinate.
