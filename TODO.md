...
General issues with stream output operators not respecting field width. Consider introducing a helper mechanism that will offer direct output to stream buffer, and automatically select between the stream buffer of the stream, or a temporarily created stream buffer backed by an memory buffer for the case where a non-zero field width is specified.
