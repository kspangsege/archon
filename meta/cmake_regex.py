...
def compile_(cmake_regex_string: str) -> Regex:
    ...
    python_regex_string = regex.format_as_python_regex(expression)
...
def _parse(cmake_regex_string: str) -> regex.Expression:
    tokens = _tokenize(cmake_regex_string)
    token = next(tokens)

    def parse_alternation() -> regex.Expression:
        alternatives = []
        while True:
        ...
        return regex.Alternation(alternatives)

    def parse_sequence() -> regex.Expression:
        elements = []
        ...
        if len(elements) == 1:
            return elements[0]
        return regex.Sequence(elements)

    def parse_repetition() -> regex.Expression:
        ...
        if max_ is None and regex.is_nullable(expression):
            raise SyntaxError(token.pos, "Unbounded repetition of nullable subexpression")
        advance()
        return regex.Repetition(expression, min_, max_)

    def parse_primary() -> regex.Expression:
        if isinstance(token, _StringToken):
            string = token.string
            advance()
            return regex.Literal(string)
        ...
        if isinstance(token, _AnchorToken):
            anchor = token.anchor
            advance()
            return regex.Anchor(anchor)
        if isinstance(token, _LParenToken):
            return parse_group()
        assert False

    def parse_group() -> regex.Expression:
    ...
    def parse_char_class(pos: int, string: str) -> regex.CharClass:
        for i, char in enumerate(string):
            if ord(char) > 127:
                raise SyntaxError(pos + i, "Non-ASCII character not supported in character class (%s)",
                                  base.quote(char))
        inverted = False
        items: list[regex.Item] = []
        i = 0
        n = len(string)
        if i < n and string[i] == "^":
            inverted = True
        ...
        while i < n:
            ...
            while True:
                ...
                if first_ord > last_ord:
                    raise SyntaxError(pos + i, "Invalid range (%s)", base.quote(string[i:i+3]))
                if continuation:
                    first_ord += 1
                if first_ord <= last_ord:
                    items.append(regex.Range(chr(first_ord), chr(last_ord)))
                ...
                i += 3
                break
        return regex.CharClass(inverted, items)

    expression = parse_alternation()
    if isinstance(token, _RParenToken):
        raise SyntaxError(token.pos, "Unmatched closing parenthesis")
    if isinstance(token, _QuantifierToken):
        raise SyntaxError(token.pos, "Invalid use of quantifier (%s)", base.quote(token.text))
...
_ANCHOR_MAP = {
    "^": regex.Anchor.Type.BEGIN,
    "$": regex.Anchor.Type.END,
}
