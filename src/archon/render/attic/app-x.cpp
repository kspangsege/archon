...
//namespace dom_impl = archon::DomImpl;
...
namespace
{
  ...
  struct PrivateApplicationState
  {
    void open_help_hud()
    {
      ...
      using namespace dom;
      ref<html::HTMLDocument> const doc = dialog->get_dom();
      ref<html::HTMLElement> const body = doc->getBody();
      ref<css::CSSStyleDeclaration> const body_style =
      ...
      body_style->setProperty(u16("padding"), u16("1px"), u16(""));
      body->appendChild(doc->createTextNode(u16("Help me!")));
      body->appendChild(doc->createTextNode(u16(" Now!")));
      ref<Element> const elem = doc->createElement(u16("B"));
      ref<css::CSSStyleDeclaration> const elem_style =
      ...
      ref<Element> const elem2 = doc->createElement(u16("I"));
      ref<css::CSSStyleDeclaration> const elem2_style =
    ...
    TextureDecl declare_texture(string image_path, bool repeat, TextureCache::FilterMode f)
    {
      ensure_texture_cache();
      UniquePtr<TextureSource> src(new TextureFileSource(image_path));
      GLenum const wrap = repeat ? GL_REPEAT : GL_CLAMP;
      return texture_cache->declare(src, wrap, wrap, f);
    }
    ...
    TextureDecl get_dashed_texture_decl()
    {
      if (!dashed_texture_decl) {
        ...
        Image::Ref const img = Image::copy_image_from(buffer, 2, 1, ColorSpace::get_Lum(), true);
...
/*
    ...
    {
      if (!dom_impl) dom_impl.reset(new dom_impl::HTMLImplementation);
      return dom_impl.get();
    }
*/
    ...
    WeakPtr<PrivateApplicationState> const &get_weak_self() { return weak_self; }
...
//    dom::ref<dom_impl::HTMLImplementation> dom_impl;
  ...
  struct DialogImpl: Dialog
  {
    DialogImpl(PrivateApplicationState *s):
      state(s->get_weak_self()), is_open(false), dirty(true), disp_list(0) {}
  ...
  struct ModalHudDialogImpl: DialogImpl //, dom_impl::Renderer
  {
    void show()
    {
...
//    dom::ref<dom::html::HTMLDocument> get_dom() { return dom_doc; }
    dom::ref<dom::html::HTMLDocument> get_dom() { return dom::ref<dom::html::HTMLDocument>(); }

    static SharedPtr<ModalHudDialogImpl> create(PrivateApplicationState *s, double dpcm)
    {
      SharedPtr<ModalHudDialogImpl> const d(new ModalHudDialogImpl(s, dpcm));
      d->weak_self = d;
      return d;
    }
    virtual void filled_box(int x, int y, int width, int height, PackedTRGB color)
    {
      std::cerr << "rect(" << x << ", " << y << ", " << width << ", " << height << ")" << std::endl;
      Vec4F rgba;
      PackedTRGB::unpack(color, rgba);
      glColor4f(rgba[0], rgba[1], rgba[2], rgba[3]);
      int const x1 = x;
      int const x2 = x1 + width;
      int const y2 = viewport_height - y;
      int const y1 = y2 - height;
      glBegin(GL_QUADS);
      glVertex2i(x1, y1);
      glVertex2i(x2, y1);
      glVertex2i(x2, y2);
...
/*
    virtual void border_box(int x, int y, int width, int height, Border const *sides)
    {
      int const ox1 = x;
      int const ox2 = ox1 + width;
      int const oy2 = viewport_height - y;
      int const oy1 = oy2 - height;

      Border const &top    = sides[0];
      Border const &right  = sides[1];
      Border const &bottom = sides[2];
      Border const &left   = sides[3];

      int const ix1 = ox1 + left.width;
      int const ix2 = ox2 - right.width;
      int const iy1 = oy1 + bottom.width;
      int const iy2 = oy2 - top.width;
...
/*
    template<int side_idx> void render_border(Border const &side,
                                              int s0, int s1, int s2, int s3, int t0, int t1)
    {
      if (!side.width || side.style == dom_impl::borderStyle_None) return;

      Vec4F rgba;
      PackedTRGB::unpack(side.color, rgba);
      glColor4f(rgba[0], rgba[1], rgba[2], rgba[3]);
      ...
      if (side.style == dom_impl::borderStyle_Dashed) {
        if (!dashed_texture) dashed_texture = dashed_texture_decl.acquire();
        dashed_texture.bind();
        len = 2*3*side.width;
      }
      else {
        if (!dotted_texture) dotted_texture = dotted_texture_decl.acquire();
        dotted_texture.bind();
        len = 2*side.width;
      }
...
//    dom::ref<dom_impl::HTMLDocument> dom_doc;
    ...
    int viewport_height;
    TextureDecl const dashed_texture_decl;
    TextureUse dashed_texture;
    TextureDecl const dotted_texture_decl;
...
/*
                                                                 ...
                                                                 PackedTRGB::CssLevel css_level)
  {
    ...
    dom::ref<HTMLDocument> const doc(new HTMLDocument(s->get_dom_impl(),
                                                      HTMLDocument::mode_HTML_Strict));
    dom::ref<dom::Element> const root = doc->createElement(dom::str_from_cloc(L"HTML"));
...
/*
    ...
    int const width  = dom_doc->get_root_box_width();
    int const height = dom_doc->get_root_box_height();
    int const x = (viewport_width  - width)  / 2;
    int const y = (viewport_height - height) / 2;
...
//              cout << "Lagging frames" << endl;
    ...
    {
      TextureCache::FilterMode const filter_mode =
    ...
    Application::Application(string title, Config const &cfg, locale const &loc, Connection::Arg c,
      ...
      need_misc_update(false), projection_needs_update(true),
...
//      cout << "Direct rendering context: " << (ctx->is_direct() ? "Yes" : "No") << endl;
    ...
    Application::~Application()
    {
      ...
      if (status_hud_disp_list) glDeleteLists(status_hud_disp_list, 1);
    }
