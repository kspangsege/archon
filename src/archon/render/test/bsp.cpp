...
template<class Scene> struct BSP_Tree {
    template<class Iter> BSP_Tree(std::size_t num_components, Iter polygons_begin, Iter polygons_end):
        num_components(num_components),
        root(build(polygons_begin, polygons_end))
    {
    }

    template<class Op> void traverse(const T* origin, const T* dir, const Op& op, T cutoff_angle, T cutoff_dist)
    {
...
private:
    ...
    const Node* build()
    {
        for (Iter i = polygons_begin; i != polygons_end; ++i)
            add_poly(*i);
    ...
    template<class Op> struct Traverse {
        ...
        {
            T dist = math::vec_dot(node->normal, node->normal + num_components, origin) + node->dist;
            T angle = std::acos(math::vec_dot(node->normal, , dir));
            if (dist > 0) {
                // Origin in front of the split plane
                if (node->back && angle < M_PI/2+cutoff_angle && dist < std::cos(angle-cutoff_angle)*cutoff_dist)
                    traverse(node->back);
                op(node->polygon);
                if (node->front)
                    traverse(node->front);
                ...
                if (node->front && M_PI/2-cutoff_angle < angle && 0 < dist+cos(M_PI-angle-cutoff_angle*M_PI/180)*cutoff_dist)
                    traverse(node->front);
                if (node->back)
                    traverse(node->back);
            }
        ...
        Traverse(const T* origin, const T* dir, Op op, T cutoff_angle, T cutoff_dist):
            origin(origin),
            dir(dir),
            op(op),
    ...
    private:
        ...
        const T* const origin;
        const T* const dir;
        const Op op;
        const T cutoff_angle; // Half the outer field of view
        const T cutoff_dist;  // Distance from eye to far clipping plane along cone edge
    };
...
struct PolygonSet {
    ...
    std::vector<std::size_t> vertices; // Indices into 'points'.
    std::vector<std::size_t> polygons; // One plus index of last vertex of polygon.
};
*/
...
/// A N-dimensional description of an (N-1)-dimensional hypersurface constructed
/// from a set of (N-1)-simplices. This could for example be the boundary of an
/// N-dimensional polytope. The surface has orientation, so the notions of front
/// and back are well defined.
///
...
template<int N> struct FacetedSurface {
    ...
    /// (N-2)-subsimplices.
    ///
    /// An (N-1)-simplex is composed of N subsimplices. The subsimplices are
    /// (N-2)-simplices. For example, a triangle, which is a 2-simplex, is
    /// composed of 3 line segments, which are 1-simplices.
    ///
    /// The i'th subsimplex arise from the vertices as follows:
    ///
    /// 2-D: Line segment consisting of two points: The i'th point is the i'th
    /// vertex.
    ///
    ///         point
    ///         -----
    /// vertex | 0 1
    ///
    /// 3-D: Triangle consisting of 3 line segments: The i'th oriented line
    /// segment runs from i'th vertex to the (i+1)'th vertex.
    ///
    ///      line segment
    ///         -------
    /// vertex | 0 1 2
    ...
    /// Orientation:
    ///
    /// The front side of a facet is the side that its normal points to. Its
    /// normal is defined as the (N-1)-ary generalized cross product of the
    /// ordered set of vectors (v[i] - v[0]) where v[i] =
    /// vertices[vertex_indices[i]], with 'i' running from 1 to N-1. It can also
    /// be defined as the Hodge dual of the wedge product of those vectors.
    ///
    /// See http://en.wikipedia.org/wiki/Cross_product#Multilinear_algebra
    ///
    /// The generalized cross product should probably be implemented as a
    /// function that takes an (N-1)-by-N matrix as argument M with the (v[i] -
    /// v[0]) as row i+1. The function should then compute the vector V where
    /// V[i] = (-1)^(N-1+i) * det(del_col(M,i)) where i is in [0;N-1] and
    /// del_col(M,i) is the result of deleting the (i+1)'th column from M.
    struct Facet {
        std::size_t vertex_indices[N];

        /// The i'th neighbor is the index of the neighboring facet across the
        /// specific unique (N-2)-subfacet of this facet that does not involve
        /// the i'th vertex of this facet.
        std::size_t neighbors[N];
    };

    using Vertex = BasicVec<N>;
    std::vector<Vertex> vertices;
    std::vector<Facet> facets;
};


using TriangleSurface = FacetedSurface<3>;
...
struct CSG {
    ...
    {
        // First compute the polygon that arises as the intersection between the
        // specified plane and the axis aligned bounding box of the CSG.
        ...
        // No intersection. If everything is in front of the clipping plane,
        // everything must be discarded, otherwise, everything must be kept.
        if (front)
            root.reset();
        return;

      intersection:
        Vec3 p1 = corner(order[i-1]);
        Vec3 p2 = corner(order[i-0]);
        double f = 1 / (h2 - h1);
        Vec3 p = f*h2*p1 - f*h1*p2; // Weighted mean
...
private:
    ...
    struct DivisionNode: Node {
        const Hyperplane3 plane;
        NodePtr front, back;
        DivisionNode(const Hyperplane3& p):
            plane(p)
        {
        }
    };
    ...
    void clip(const Hyperplane3& plane, NodePtr& ptr)
    {
        // Recurse
        if (DivisionNode *div = dynamic_cast<DivisionNode*>(ptr.get())) {
            if (div->front)
                clip(plane, div->front);
            if (div->back)
                clip(plane, div->back);
            return;
        }
...
class BSP: public Application {
public:
    BSP():
        Application("archon::render::BSP")
    {
/*
        CSG csg;
