...
// Must be in the range [2;8]. Codec design/definition limits this to 2 bits
// downwards.
constexpr int min_bits_per_byte = 2;

// Must be in the range [2;8]. Compressor hash table design limits this to 8
// bits.
constexpr int max_bits_per_byte = 8;

// Must be in the range [9;12]. Compressor hash table design limits this to 12
// bits.
constexpr int max_bits_per_code = 12;

constexpr unsigned max_num_codes = 1u << max_bits_per_code;

constexpr unsigned nil_code = max_num_codes;


class LempelZivWelchCompressor {
public:
    ...
    LempelZivWelchCompressor(int bits_per_byte):
...
/// </pre>
///
/// Returns false when byte assmebler has 8 or more bits and the ouput buffer is
/// full.
///
/// Returns true when byte assmebler has less than 8 bits and the input buffer
/// is empty.
///
/// Need at least 31 bits for byte assembler (2*12+7)
///
/// The first code that is generated, is always equal to the value of the first
/// input byte.
///
/// The last code that we can register in the hash table has value 4095
/// (assuming max 12 bits per code), this code must refer to a prefix code whose
/// value is strictly less than 4095. Thus, the value 4095 is never used as a
/// prefix code when generating the hash key, so if we add one to the prefix
/// code when generating the hash key, we can use zero to detect empty entries.
///
...
class LempelZivWelchCompressor::State {
public:
    bool conv(const char*& in, const char* in_end, char*& out, char* out_end, bool eoi)
    {
        for (;;) {
            ...
            while (8 <= bits_in_assembler) {
                ...
                *out++ = unsigned(assembler) & 0xFFu;
                assembler >>= 8;
                bits_in_assembler -= 8;
            }

            // Flush the last bits in the assembler if the EOF code has been
            // added
            if (eof_code_sent) {
            ...
            for (;;) {
                ...
                // Check if the concattenation of prefix and suffix is in the
                // dictionary
                ...
                if (code == nil_code) {
                    ...
                    if (dictionary_size == max_num_codes) {
                        ...
                        hash_insert(key, dictionary_size++);
                    }
    ...
    {
        assembler |= UIntFast32(code) << bits_in_assembler;
    ...
    void add_code_and_check_dict_size(unsigned code)
    {
        ...
        // The next code can be as large as one minus the dictionary size. If
        // this is too much for the current number of bits, increase the number
        // of bits by one
    ...
    void hash_insert(UIntMin32 key, unsigned code)
    {
        ...
        for (;;) {
            UIntMin32 v = hash_table[i];
            if (!v) {
                hash_table[i] = UIntMin32(code) << 20 | key;
                return;
            }
            if (++i == hash_table_size)
                i = 0;
    ...
    {
        return (UIntMin32(prefix+1u) << 8) | suffix;
    }

    unsigned hash_index(UIntMin32 key)
    {
        return unsigned((key >> 12) ^ key) & hash_index_mask;
    }

    State(const LempelZivWelchCompressor& c):
    ...
    void reset()
    {
        ...
        std::fill(hash_table, hash_table+hash_table_size, 0);
    }

    static constexpr int bits_per_hash_index  = max_bits_per_code + 1;
    static constexpr unsigned hash_table_size = 1u << bits_per_hash_index;
    static constexpr unsigned hash_index_mask =  hash_table_size - 1u;

    const int bits_per_byte;
    const unsigned max_byte_value = (1u << bits_per_byte) - 1u;
    const unsigned clear_code = max_byte_value + 1u;
    const unsigned eof_code = clear_code + 1u;

    UIntFast32 assembler = 0;
    int bits_in_assembler = 0, bits_per_code;
    bool eof_code_sent = false;
    unsigned code_mask, prefix = nil_code, dictionary_size;
...
class LempelZivWelchDecompressor {
public:
    using source_char_type = char;
    using target_char_type = char;
    ...
    LempelZivWelchDecompressor(int bits_per_byte):
        bits_per_byte(bits_per_byte)
    {
    }
...
/// \sa http://marknelson.us/1989/10/01/lzw-data-compression/
/// \sa Nelson, M.R.: "LZW Data Compression", Dr. Dobb's Journal, October 1989.
class LempelZivWelchDecompressor::State {
public:
              ...
              char*& target_begin, char* target_end, bool eoi)
    {
        ...
        for (;;) {
            // First transfer any previously generated output to callers target
            // buffer
            if (out_stack_dirty) {
                std::size_t n = out_stack_bottom - out_stack_top, m = target_end - target_begin;
                bool partial = (m < n);
                if (partial)
                    n = m;
                if (n != 0) {
                    char* e = out_stack_top + n;
                    target_begin = std::copy(out_stack_top, e, target_begin);
                    out_stack_top = e;
                }
                if (partial)
                    return false; // Output stack not yet empty
            ...
            while (bits_in_assembler < bits_per_code) {
                ...
                assembler |= UIntFast32(c) << bits_in_assembler;
                bits_in_assembler += 8;
            }

            // Extract next code from assembler
            unsigned code = unsigned(assembler) & code_mask;
            ...
            if (code < clear_code) {
                ...
                if (target_begin < target_end) {
                    ...
                    *--out_stack_top = static_cast<unsigned char>(code);
                }
                last_suffix = static_cast<unsigned char>(code);
            }
            else {
                if (code < dictionary_size) {
                    last_suffix = push_dictionary_string(code);
                }
                else {
            ...
            // Unless the dictionary is full or this is the first code after a
            // reset, register dict(prev_code) + last_suffix in the dictionary.
            if (prev_code != nil_code && dictionary_size < max_num_codes) {
                ...
                // If dictionary size exceeds current number of bits per code,
                // then increment number of bits per code
                if (code_mask < dictionary_size && dictionary_size < max_num_codes)
                    code_mask = (1u << ++bits_per_code) - 1u;
            }
    ...
    State(const LempelZivWelchDecompressor& d):
        bits_per_byte(d.bits_per_byte)
    {
        reset();
    }

    const int bits_per_byte;
    const unsigned clear_code = 1u << bits_per_byte;
    const unsigned eof_code = clear_code + 1;
    const unsigned first_compound_code = eof_code + 1;

    bool out_stack_dirty = false, eof_code_seen = false;
    UIntFast32 assembler = 0;
    int bits_in_assembler = 0, bits_per_code;
    unsigned code_mask, prev_code, dictionary_size;
    char last_suffix;

    /// Together with <tt>suffix</tt>, this table defines the dictionary in the
    /// following way:
    ///
    /// <pre>
    ///
    ...
    /// The maximum number of bytes we need to be able to push to the stack is 1
    /// + the longest possible string in the dictionary, which can be derived
    /// using:
    ///
    /// <pre>
    ///
    ...
    /// </pre>
    ///
    /// Since <tt>code < max_num_codes</tt> and <tt>2**min_bits_per_byte + 2 <=
    /// first_compound_code</tt>, the maximum length of a dictionary string must
    /// be <tt>max_num_codes - 2**min_bits_per_byte - 1</tt>.
    char out_stack[max_num_codes - (1u<<min_bits_per_byte)];

    char* const out_stack_bottom = out_stack + sizeof out_stack;
    char* out_stack_top = out_stack_bottom;
};
...
std::unique_ptr<const Codec> get_lempel_ziv_welch_codec(int bits_per_byte)
{
    if (bits_per_byte < min_bits_per_byte || max_bits_per_byte < bits_per_byte)
        throw std::invalid_argument("'Bits per byte' is out of range");
    using Enc = LempelZivWelchCompressor;
    using Dec = LempelZivWelchDecompressor;
