...
*/
    ...
    /**
     * An abstract material with the ability to map texture
     * coordinates to surface properties and compute the color seen by
     * an incident ray due to a specific light source.
     */
    struct Material
    {
      /**
       ...
       * Must be thread-safe.
       *
       * \param texture_point The texture coordinates at the shaded
       * surface point.
       *
       * \param normal The surface normal at the shaded surface
       * point. It must be a vector of unit length.
       *
       * \param view_dir The direction from the shaded surface point
       * towards the view point. It must be a vector of unit length.
       *
