...
namespace
{
  ...
  void test_1()
  {
    ...
    //pixelFormat.wordType = PixelFormat::std_double;
    //pixelFormat.bitsPerWord = 128;
    pixelFormat.pixelSize = 3;
    pixelFormat.colorScheme = PixelFormat::luminance;
    ...
    endianness[1] = false;
    endianness[0] = false;
    ImageData image(buffer, 2, 2, pixelFormat, bufferFormat, 0, 0, 0, 0, endianness);
    long double tray[64];
    for(size_t i=0; i<sizeof(tray)/sizeof(*tray); ++i) tray[i] = -3.1415;

    image.getPixels(0, 0, tray, 32, 1);

    for(size_t i=0; i<sizeof(tray)/sizeof(*tray); ++i) cerr << i << ": " << tray[i] << endl;
  }
  ...
  void test_8()
  {
    ...
    Array<long double> pixel(image1.getComponentSpecifier());
