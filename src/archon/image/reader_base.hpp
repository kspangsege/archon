...
/// This class provides all the low-level reading features that are common to an
/// <tt>ImageReader</tt> and and <tt>ImageWriter</tt>.
class ReaderBase {
protected:
    using TupleGrid = util::TupleGrid;
    using ConstTupleGrid = util::ConstTupleGrid;
    using PixelFormat = PixelConverter::Format;
    ...
    void set_pos_align(double x, double y)
    {
        pos_align_x = x;
        pos_align_y = y;
    }
    ...
    void set_falloff(Falloff h, Falloff v)
    {
        horiz_falloff = h;
        vert_falloff = v;
    }
    ...
    template<class T, bool has_alpha> void get_pixel_smart(T* p, const ColorSpace* c);

    /// \param c If null, RGB is assumed.
    template<bool has_alpha> void get_pixel(void* p, const ColorSpace* c, WordType t);

    void get_block(TupleGrid g, const PixelFormat& f, int width, int height);
    ...
    template<class Op> static void subdivide_block_op(Op&, int width, int height,
                                                      std::size_t max_pixels_per_subblock);
    ...
    // Simply a trick used by WriterOps::put_image to get access to various
    // protected attributes of the ImageReader argument.
    template<class WriterBase>
    static void put_image(const ReaderBase& r, WriterBase& w, int width, int height)
    {
    ...
    const Image::ConstRef image;
    const int image_width, image_height; // Actual dimmensions of accessed image
    const PixelFormat pixel_format;
    const WordType best_float; // The floating point type that best covers pixel_format.word_type
    int bytes_per_best_float;
    const ColorSpace::ConstRef rgb;
    Image::CodecConstPtr codec;

    PixelConverter::Buffers buffers;
    bool read_cvt_initialized = false;
    bool write_cvt_initialized = false; // Placed here so that it can be cleared when backgound color changes
...
private:
    std::unique_ptr<char[]> color_slot_buffer;

    double pos_x, pos_y, pos_align_x, pos_align_y;

    template<bool noop, bool dense> class ReadOp;
    using NoopReadOp = ReadOp<true,  false>;
    using DenseReadOp = ReadOp<false, true>;
    using GridReadOp = ReadOp<false, false>;

    void prep_read_cvt(const PixelFormat&);
    ...
    void get_restricted_block(TupleGrid g, const PixelFormat& f, int x, int y, int w, int h);

    void clear_tray(TupleGrid g, const PixelFormat& f, int w, int h);

    void store_color(const char* b, const PixelFormat& f, bool foreground);

    void cvt(const PixelFormat& source_format, const char* source,
             const PixelFormat& target_format, char* target, std::size_t n);

    static constexpr std::size_t s_num_color_slots = 2;

    PixelConverter read_cvt;
    TransferFormat read_cvt_format;

    // Only initialized when not noop and assumes dense operation
    std::size_t read_cvt_max_pixels_per_block;


    class ReaderCvtStepsBase {
    public:
        virtual bool is_bg_clean(const ReaderBase*) const = 0; // True if transparent or black
        virtual void background_blend(const ReaderBase*, const char*, char*, std::size_t) const = 0;
        virtual ~ReaderCvtStepsBase() {}
    };

    template<class T, WordType> class ReaderCvtSteps: public ReaderCvtStepsBase {
    public:
        bool is_bg_clean(const ReaderBase*) const override;
        void background_blend(const ReaderBase*, const char*, char*, std::size_t) const override;
        const ReaderCvtStepsBase* operator()() const
        {
            return this;
        }
    };

    class ReaderCvtStepsSwitch:
        public WordTypeSwitch<ReaderCvtSteps, void, const ReaderCvtStepsBase*, true> {};

    static ReaderCvtStepsSwitch s_reader_cvt_steps_switch;

    const ReaderCvtStepsBase* const reader_cvt_steps;

protected:
    // NOTE: It is the responsibility of the user to ensure that neither of the
    // two methods are called until after prep_color_slots() has been called for
    // the associated ReaderBase.
    class BackgroundBlender: public PixelConverter::Manipulator {
    public:
        ...
        BackgroundBlender(const ReaderBase* r):
            reader{r}
        {
        }
        const ReaderBase* const reader;
    };

    const BackgroundBlender background_blender;
};
...
template<bool noop, bool dense> class ReaderBase::ReadOp {
public:
    void operator()(int x, int y, int w, int h) const
    {
        ...
        const PixelConverter& cvt = reader->read_cvt;
    ...
    ReadOp(ReaderBase* r, const TupleGrid& g, int x0, int y0):
        reader(r),
        grid(g),
        x0(x0),
        y0(y0)
    {
        ...
        if (!noop && !dense)
            reader->read_cvt.ensure_internal_target(reader->buffers);
    }

private:
    ReaderBase* const reader;
    const TupleGrid& grid;
    const int x0, y0;
};
...
inline ReaderBase::ReaderBase(Image::ConstRefArg image):
    ...
    bytes_per_best_float(get_bytes_per_word(best_float)),
    rgb(ColorSpace::get_RGB()),
    buffers(std::max((ColorSpace::max_num_primaries + 1) * get_max_bytes_per_word(), 2048)),
    reader_cvt_steps(s_reader_cvt_steps_switch(best_float)),
    background_blender(this)
{
...
inline void ReaderBase::get_pixel_smart(T* p, const ColorSpace* c)
{
    if (has_alpha && (pixel_format.format.has_alpha || read_cvt_initialized &&
        ...
        if (has_alpha)
            p[c ? c->get_num_primaries() : 3] = util::frac_full<T>();
    }
}


template<bool has_alpha>
inline void ReaderBase::get_pixel(void* p, const ColorSpace* c, WordType t)
{
...
inline void ReaderBase::get_block(TupleGrid g, const PixelFormat& f, int w, int h)
{
    // Determine falloffs
    ...
    if (!read_cvt_initialized || f.format != read_cvt_format)
        prep_read_cvt(f);

    int x = get_block_pos_x(w), y = get_block_pos_y(h);
    int falloff_left   = clip_left   - x, falloff_right = x + w - clip_right;
    int falloff_bottom = clip_bottom - y, falloff_top   = y + h - clip_top;

    bool horiz = (0 < falloff_left   || 0 < falloff_right);
    bool vert  = (0 < falloff_bottom || 0 < falloff_top);
    ...
    int clip_width = clip_right - clip_left, clip_height = clip_top - clip_bottom;
    bool empty_clip = clip_width < 1 || clip_height < 1;
    ...
    if (horiz || vert) {
        int proto_width  = w + w2;
        int proto_height = h + h2;
...
{
    using MoveRight = void (TupleGrid::*)(int width);
    MoveRight move_right = (vert ? &TupleGrid::move_up : &TupleGrid::move_right);
    int clip_left  = (vert ? this->clip_bottom : this->clip_left);
    int clip_right = (vert ? this->clip_top    : this->clip_right);
    ...
    switch (vert ? vert_falloff : horiz_falloff) {
        case falloff_Background:
            ...
            if (0 < falloff_right)
                w -= falloff_right;
            if (w < 1)
                return true; // Nothing is left
            break;

        case falloff_Edge:
            if (0 < falloff_left) {
                if (falloff_left < w) {
                    rep_left2 = falloff_left;
                }
                else {
        ...
        case falloff_Repeat: {
            int l = core::modulo(falloff_left, clip_width);
            int l2 = (l ? l : clip_width);
            if (w <= l2) {
                // Tray spans only a single clipping module, so slide it
                // horizontally by an integer multiple of the clipping width
                // such that it falls within the clipping region.
                x = clip_right - l2;
            }
            else {
                int r = w - l - clip_width;
                if (0 <= r) {
                    // Horizontally we have a complete clipping module within
                    // the tray.
                    rep_left1 = l;
                    rep_right1 = r;
                    w = clip_width;
                    ...
                    // Tray spans two clipping modules horizontally but does not
                    // contain a complete module, so we are forced to break the
                    // operation into two pieces.
                    if (clip_width < w) {
...
inline void ReaderBase::get_restricted_block(TupleGrid g, const PixelFormat& f,
                                             int x, int y, int w, int h)
{
    ...
    // final step that spreads the converted pixels.
    if (dense_grid && n <= read_cvt_max_pixels_per_block) {
        DenseReadOp(this, g, x, y)(0, 0, w, h);
    }
    else {
...
inline void ReaderBase::prep_read_cvt(const PixelFormat& f)
{
    prep_color_slots();
...
template<class T, WordType w>
inline bool ReaderBase::ReaderCvtSteps<T,w>::is_bg_clean(const ReaderBase* r) const
{
    const T* rgba = reinterpret_cast<T*>(r->get_rgba_ptr(false));
    return (rgba[3] == 0 || (rgba[0] == 0 && rgba[1] == 0 && rgba[2] == 0));
}


template<class T, WordType w>
inline void ReaderBase::ReaderCvtSteps<T,w>::background_blend(const ReaderBase* r,
                                                              const char* src, char* tgt,
                                                              std::size_t n) const
{
    const T* s = reinterpret_cast<const T*>(src);
    T* t = reinterpret_cast<T*>(tgt);
    const T* bg_rgba = reinterpret_cast<T*>(r->get_rgba_ptr(false));
    T bg[3] = {
        bg_rgba[0] * bg_rgba[3],
        bg_rgba[1] * bg_rgba[3],
        bg_rgba[2] * bg_rgba[3]
...
void ReaderBase::subdivide_block_op(Op& op, int width, int height,
                                    std::size_t max_pixels_per_subblock)
{
...
// Get either background or foreground color expressed in RGBA using one
// 'best_float' per component. NOTE: It is the responsibility of the caller to
// ensure that this method is not called until after prep_color_slots() has been
// called.
inline char* ReaderBase::get_rgba_ptr(bool foreground) const
{
    char* p = color_slot_buffer.get();
    if (foreground)
        p += 4*bytes_per_best_float;
    return p;
}


// Get a pointer to either the backgound or the foreground color. The pixel
// format will be identical to that of the held image codec. Both internal
// buffers may get clobbered.
inline char* ReaderBase::get_color_ptr(bool foreground) const
{
    ...
    if (foreground)
        p += pixel_format.bytes_per_pixel;
    return p;
}
...
{
    if (color_slot_buffer)
        return;
    color_slot_buffer =
        std::make_unique<char[]>(2 * 4 * bytes_per_best_float +
                                 s_num_color_slots * pixel_format.bytes_per_pixel); // Throws
    set_color(util::color::transparent, false); // Background color
    set_color(util::color::black, true);        // Foreground color
}


inline void ReaderBase::store_color(const char* b, const PixelFormat& f, bool foreground)
{
...
inline void ReaderBase::clear_tray(TupleGrid g, const PixelFormat& f, int w, int h)
{
...
inline void ReaderBase::cvt(const PixelFormat& f, const char* s,
                            const PixelFormat& g, char* t, std::size_t n)
{
    PixelConverter c(f, g, buffers);
    if (c.is_noop()) {
        std::copy(s, s + n*f.bytes_per_pixel, t);
    }
    else {
        c(s,t,n);
    }
