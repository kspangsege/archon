...
namespace
{
  ...
  void cvt_frac(void const *source, void *target, size_t n)
  {
    Source const *s = static_cast<Source const *>(source);
    Target       *t = static_cast<Target       *>(target);
    frac_any_to_any(s, t, n);
  }

  template<typename Source, typename Target>
  void cvt_clamp(void const *source, void *target, size_t n)
  {
    Source const *s = static_cast<Source const *>(source);
    Target       *t = static_cast<Target       *>(target);
    for(size_t i=0; i<n; ++i) t[i] = clamp_any_to_any<Target, Source>(s[i]);
  }
  ...
  struct Cvts
  {
    ...
    Cvts() {}
    Cvts(WordTypeConverter frac, WordTypeConverter clamp): frac(frac), clamp(clamp) {}
  };
  ...
  struct WordTypeDescriptor
  {
    ...
    WordTypeDescriptor(WordType type, string tag, int width, bool is_float):
      type(type), tag(tag), width(width), is_float(is_float) {}
  };
  ...
  struct WordTypeRegistry
  {
    ...
    template<typename Source, typename Target> void add_cvt(WordTypeDescriptor &d, WordType target)
    {
      vector<Cvts> &v = d.converters;
      if(v.size() <= unsigned(target)) v.resize(target+1);
      v[target] = Cvts(&cvt_frac<Source, Target>, &cvt_clamp<Source, Target>);
    }

    template<typename T> void add_descriptor(WordType type, string tag)
    {
      if(types.size() <= unsigned(type)) types.resize(type+1);
      ...
      else int_width_map[d.width] = type;
    }
  ...
  inline WordTypeRegistry const &get_word_type_registry()
  {
    static WordTypeRegistry registry;
    return registry;
  }

  inline WordTypeDescriptor const &get_word_type_descriptor(WordType type)
  {
...
namespace Archon
{
  namespace Imaging
  {
    ...
    {
      map<int, WordType> const &m = floating_point ?
    ...
    {
      if(at_least) return get_word_type_by_minimum_bit_width(width, floating_point);
    ...
    {
      map<int, WordType> const &m = get_word_type_registry().mant_width_map;
    ...
    {
      map<string, WordType> const &m = get_word_type_registry().tag_map;
