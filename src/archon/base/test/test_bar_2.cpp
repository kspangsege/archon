...
void inc_decode(base::Span<const char> data, std::size_t& data_offset,
                base::Span<char> buffer, std::size_t& buffer_offset, bool end_of_data) noexcept;
...
public:
    ...
    [[nodiscard]] bool try_read(base::Span<char> buffer, std::size_t& n, std::error_code&,
                                bool dynamic_eof = false) noexcept;

    [[nodiscard]] bool try_write(base::Span<const char> data, std::size_t& n,
                                 std::error_code&) noexcept;

    [[nodiscard]] bool try_revert(std::size_t decoded_offset, std::error_code&) noexcept;
...
private:
    base::File& m_file;
    std::size_t m_last_read_size = 0; // Zero in neutral and writing modes
#if ARCHON_DEBUG
...
public:
    ...
    // had been decoded_offset, and the read operation had resulted in a
    // situation where no data was pre-loaded.
    //
    // When in reading mode, decoded_offset must be less than, or equal to the
    // size of the extracted chunk during the last invocation of try_read(),
    // which is zero if that read operation failed. When in neutral mode,
    // decoded_offset must be zero.
    //
    [[nodiscard]] bool try_revert(std::size_t decoded_offset, std::error_code&) noexcept;
...
private:
    ...
    std::size_t m_prev_begin = 0; // Zero in neutral and writing modes
...
inline bool PrimitivePosixTextFileCore::try_read(base::Span<char> buffer, std::size_t& n,
                                                 std::error_code& ec, bool) noexcept
{
#if ARCHON_DEBUG
    ...
    n = 0;
    bool success = m_file.try_read_some(buffer, n, ec);
    m_last_read_size = n;
    return success;
}
...
inline bool PrimitivePosixTextFileCore::try_revert(std::size_t decoded_offset,
                                                   std::error_code& ec) noexcept
{
#if ARCHON_DEBUG
    ARCHON_ASSERT(!m_writing);
    ...
    ARCHON_ASSERT(decoded_offset <= m_last_read_size);
    std::size_t n = std::size_t(m_last_read_size - decoded_offset);
...
bool PrimitiveWindowsTextFileCore::try_read(base::Span<char> buffer, std::size_t& n,
                                            std::error_code& ec, bool dynamic_eof)
{
#if ARCHON_DEBUG
    ARCHON_ASSERT(!m_writing);
    m_reading = true;
    ...
    for (;;) {
        ...
        ARCHON_ASSERT(m_begin == m_prev_begin);
        ARCHON_ASSERT(!end_of_file);
        // Move any partial character to start of buffer
...
bool PrimitiveWindowsTextFileCore::try_revert(std::size_t decoded_offset,
                                              std::error_code& ec) noexcept
{
#if ARCHON_DEBUG
    ARCHON_ASSERT(!m_writing);
    ...
    bool success = base::newline_crlf::simulate_inc_decode(data, data_offset, decoded_offset);
...
public:
    ...
    // try_read() had been decoded_offset, and the read operation had resulted
    // in a situation where no data was pre-loaded.
    //
    // When in reading mode, decoded_offset must be less than, or equal to the
    // size of the extracted chunk during the last invocation of try_read(),
    // which is zero if that read operation failed. When in neutral mode,
    // decoded_offset must be zero.
    //
    [[nodiscard]] bool try_revert(std::size_t decoded_offset, std::error_code&);
    ...
    for (;;) {
        ...
        if (!error) {
            ...
            if (ARCHON_UNLIKELY(m_end == m_buffer.size()))
                expand_buffer(); // Throws
            std::size_t n_2 = 0;
            if (ARCHON_LIKELY(m_primitive_core.try_read(base::Span(m_buffer).subspan(m_end), n_2, ec, dynamic_eof))) {
                if (ARCHON_LIKELY(n_2 > 0)) {
                    m_end += n_2;
                    continue;
                }
...
template<class C, class T, class P>
bool TextFileCore<C, T, P>::try_revert(std::size_t decoded_offset, std::error_code& ec)
{
#if ARCHON_DEBUG
    ARCHON_ASSERT(!m_writing);
#endif
    ...
    std::size_t data_offset = m_prev_begin;
    bool success = m_codec.simulate_inc_decode(state, data, data_offset, decoded_offset); // Throws
    ARCHON_ASSERT(success);
    ARCHON_ASSERT(data_offset <= m_begin);
    std::size_t n = std::size_t(m_end - data_offset);     
    if (ARCHON_LIKELY(m_primitive_core.try_revert(n, ec))) {
        m_prev_begin = 0;
        m_begin = 0;
        m_end   = 0;
#if ARCHON_DEBUG
